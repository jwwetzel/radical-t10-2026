...
void EnergyScanDSB1()
{
  ...
  FILE *sum = fopen("Output/scan_dsb1/EnergyScanDSB1_summary.txt", "w");
  ...
  for (int ip = 0; ip < NP; ++ip) {
    ...
    hS[ip] = new TH1F(Form("hS%d",ip), ";#Sigma LG [ADC-eq];fraction / bin", 320, 0, 16000);  // DSB1: ~2.1x the LuAG response (~1390 ADC-eq/GeV)
    ...
    g->SetParLimits(1, SMIN + 30, 13400);          // mean stays on the physical peak, below the ~13.6k LG headroom ceiling
  ...
  c.SaveAs("Output/scan_dsb1/EnergyScanDSB1.png");
  fclose(sum);
