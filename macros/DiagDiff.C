...
void DiagDiff(int mod = 0)
{
  ...
  double FL[3][6] = {{180,300,420,540,660,3800},{180,1500,2500,4000,6000,6500},{180,500,1200,1700,2200,2500}};  // EJ199 floors now MATCH EnergyScanEJ199.C (audit F: prior 540/660/780 was a different selection)
  ...
  for (int ip = 0; ip < NRUNS[mod]; ++ip) {
    ...
    for (Long64_t i = 0; i < nEnt; ++i) {
      ...
      { double Scal = 0; Pulse lv[4], hv[4];
        ...
        if (Scal > 300)   // miss-class boundary, NOT the containment floor: calibration needs the full on-module LG range
